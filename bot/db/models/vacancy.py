...
from .company import CompanyDataModel, company_postgres_to_model
...
class VacancyDataModel(Base):
    ...
    company: Mapped["CompanyDataModel"] = relationship(
        primaryjoin=CompanyDataModel.id == company_id,
...
def vacancy_postgres_to_model(data_model: VacancyDataModel) -> Vacancy:
    return Vacancy(
        ...
        published=data_model.published,
        company=company_postgres_to_model(data_model.company)
    )
